...
class TestHelperFunctions(unittest.TestCase):
    ...
    def test_name_similarity_suffix_ignored(self):
        sim = _name_similarity("卓朗科技股份有限公司", "万科企业股份有限公司")
        self.assertLess(sim, 50)

    def test_name_similarity_related(self):
        sim = _name_similarity("卓朗科技股份有限公司", "天津卓研咨询有限公司")
        self.assertLess(sim, 50)  # 不靠fuzz，靠子策略

    def test_extract_names(self):
        result = _extract_names(["张某某(30%)", "李某某(25%)", "天津卓朗控股有限公司(45%)"])
        self.assertEqual(result, {"张某某", "李某某", "天津卓朗控股有限公司"})
...
class TestDimensionFunctions(unittest.TestCase):
    ...
    def test_D02_common_shareholders(self):
        ...
        # #2 和 #3 共享股东梁某
        self.assertTrue(results["天津事达科技有限公司"][0])
        self.assertTrue(results["事达信息咨询有限公司"][0])
        # 万科不应触发
        self.assertFalse(results["万科企业股份有限公司"][0])

    # ---- D03 实际控制人 ----
    def test_D03_actual_controller_triggered(self):
        t = target_map["北辰区明锐达科技有限公司"]
    ...
    def test_D08_shared_contact_triggered(self):
        ...
        # #1 和 #5 共用 138****5678
        self.assertTrue(results["温岭乾民贸易有限公司"][0])
        self.assertTrue(results["天津朗信科技有限公司"][0])

    def test_D08_shared_contact_clean(self):
        results = check_D08_shared_contact(targets)
        # 修复后 email 域名已不同，万科不触发
        self.assertFalse(results["万科企业股份有限公司"][0])
...
class TestZhuolangEndToEnd(unittest.TestCase):
    ...
    def test_vanke_clean(self):
        ...
        d05 = check_D05_legal_rep_cross(targets)
        d08 = check_D08_shared_contact(targets)
        flags.append(d02["万科企业股份有限公司"][0])
        flags.append(d05["万科企业股份有限公司"][0])
        flags.append(d08["万科企业股份有限公司"][0])

        total = sum(flags)
        self.assertEqual(total, 0, f"万科应全✗，实际触发 {total} 个维度: {flags}")

    def test_D08_shared_phone(self):
        """D08 应捕获电话共用（温岭乾民 + 天津朗信共用一个号码 138****5678）"""
        d08 = check_D08_shared_contact(targets)
        self.assertTrue(d08["温岭乾民贸易有限公司"][0])
        self.assertTrue(d08["天津朗信科技有限公司"][0])

    def test_D06_captures_trajectory(self):
        """D06 应捕获法人变更轨迹（乾民信息前法人赵某=审计方监事）"""
        triggered, reason = check_D06_legal_rep_trajectory(audit, target_map["乾民信息技术有限公司"])
        self.assertTrue(triggered)
        self.assertIn("赵某", reason)
